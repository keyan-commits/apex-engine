...
**Last updated:** 2026-05-28 (late)
**Last action:** Post-Wave 23 session — no code changes, three substantive activities. (1) **Wave 23 feedback loop closed end-to-end**: ran `pnpm catalog:check --emit`, the live `gemini-3.5-flash` candidate finding wrote a `recordAutoImprovement` record (signature `4c8399486dac00a5`), `pnpm feedback:flush` converted it to **GH issue #35** with `[apex-engine] [improvement] [auto] [catalog]` prefix and proper `feedback`+`enhancement` labels. End-to-end: catalog probe → outbox JSON → GH issue in two keystrokes. (2) **meeting-decoder dogfood**: `yt-dlp` pulled Luke Alvoeiro's Factory.ai talk "The Multi-Agent Architecture That Actually Ships" (https://www.youtube.com/watch?v=ow1we5PzK-o, ~17 MB m4a, 3s download), then `/Users/nikoe/Development/Study/meeting-decoder/.venv/bin/meeting-decoder` against it with Groq `openai/gpt-oss-120b` backend; whisper produced clean transcript in ~50s on M2 Pro, the action-items/topics LLM phase hit Groq 429 (same RPD I'd burned on apex_synthesize earlier), `--no-action-items --no-topics --no-prose` second run completed cleanly. Transcript at `/tmp/missions-talk/missions-talk/missions-talk.transcript.txt` (20 KB, 343 lines). (3) **Workflow review**: read `~/.claude/CLAUDE.md` (9-rule agentic pipeline) + observed actual session behavior, WebSearched HANDOFF.md / Claude Code subagent best practices, ran `apex_synthesize` with includeClaude=true for MoA verdict on improvements (confidence 85, 4/4 models converged). Then compared apex-engine workflow to Factory.ai Missions architecture from the decoded transcript. **Single highest-value adoption identified**: replace prose `## Last action` paragraphs with Factory's structured-handoff field-set (Completed / Left undone / Commands run + exit codes / Issues discovered / Validation contract status). Real failure pattern that this prevents: 6+ consecutive `HANDOFF.md: patch Wave NN commit SHA` fixup commits today (1c6da6d, 7840aa6, e6cfd35, 85d939c, f74ff56, 1c6da6d) — a `Commands run` field with `git rev-parse --short HEAD → <SHA>` makes the backfill automatic. Drafted cross-instance message for the other Mac's CC session (which has been working on HANDOFF improvements); user opted to copy-paste rather than file via apex_report, so no GH issue / outbox record created.

**Pending decisions** (from this session's research, no code changes yet):
- **Structured handoff schema adoption** — the other Mac's CC session is working on this; recommendation sent via direct copy-paste.
- **Tier Rule 2 by scope** — TINY (<150 LOC) → 1 Explore + apex_synthesize; STANDARD → 4-subagent phalanx; LARGE → +plan+research subagents. (`~/.claude/CLAUDE.md` change.)
- **Shape-based approval guard for Rule 9** — blanket approvals cover consecutive waves of the SAME shape only; STOP for deletes / schema changes / new deps / auth changes / category breaks.
- **Phase 4.5 DOGFOOD** (conditional) — between EXECUTE and VERIFY for waves that add tools operating on project-internal surfaces.
- **Two new memory entries**: `feedback_handoff_sha_backfill.md` + `feedback_multi_wave_shape_break.md`.
- **GH #35 (gemini-3.5-flash candidate)** — awaiting manual Production-tier / pricing / API-compat verification before bumping `gemini-2.5-flash` in `providers.ts`.

Earlier today: Wave 23 shipped — **provider catalog drift detector** (LFM ask: "I want our LLMs to be auto-updated when newer version of existing LLMs is available"). Shape decision: detect-and-notify, NOT blind auto-bump. New `src/lib/catalog-check.ts` (~250 LOC) defines `TRACKED_MODELS[]` (5 entries: `llama-3.3-70b-versatile`, `llama-3.1-8b-instant`, `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `gemini-2.5-flash`) each with a family-match regex + version comparator. Probes two catalog APIs (Groq `GET /openai/v1/models`, Google `GET /v1beta/models`) on demand; finds newer same-family candidates; files `apex_report` improvement records via `recordAutoImprovement` (1-hr in-process throttle, signature dedup keyed on `provider+model`). Reports include verification checklist (Production-tier check, pricing regression check, API contract compatibility, `pnpm test:run` after swap, update `TRACKED_MODELS` to prevent re-report). New CLI `pnpm catalog:check` (dry-run by default; `--emit` files reports). Deliberately skipped: Anthropic (Claude Code OAuth owns selection), GitHub Models (next-gen GPT families won't pattern-match), DeepSeek (`deepseek-chat` is a provider-rolled alias). **Live dry-run from this commit found `gemini-3.5-flash` as a newer candidate for `gemini-2.5-flash`** — tool works end-to-end. 27 new tests covering family matching (positive/negative/cross-provider), aggregation, formatting, and probe-failure recovery. 557/557 tests; qa:check + security:check both clean. Commit `d868a36`.

Earlier today: Wave 22f shipped — closes the missing half of LFM #33b that yesterday's apex_doc_review dogfood surfaced. The Wave 22a Gemini quota-exhaust → `llama-3.1-8b-instant` substitute was wired ONLY into the web UI SSE fan-out (`src/app/api/ask/route.ts`), not into the MCP-side `runFanOut()` that backs `apex_fanout` / `apex_synthesize` — which is the path LFM actually uses per #34. Live dogfood proved it: pre-flight correctly said "Gemini: SKIPPED — quota-exhausted" but the substitute didn't fire, Gemini's panel just dropped. **22f fix**: new `maybeSubstituteGeminiQuota(answer, composedSystem, prompt)` helper runs post-hoc inside `runFanOut` — matches (provider==="gemini" && classifyError.kind==="gemini-quota-exhausted" && !text), fires `streamGeminiQuotaFallback`, replaces the slot with `model: GEMINI_QUOTA_FALLBACK_MODEL` + `substituted: {from, reason}`. `formatAnswers` now renders the substituted state distinctly ("## Gemini — llama-3.1-8b-instant (substituted, was gemini-2.5-flash)" + original-failure note). Same env gate (`APEX_GEMINI_QUOTA_FALLBACK !== "skip"`). Implementation note: post-hoc rather than concurrent — costs one extra round-trip when substituting (only on quota-exhaust, small fraction of calls) in exchange for zero plumbing changes to `fanOut()` / `collectStream()` / `FanOutItem`. **Preflight updated**: `[PRE-FLIGHT STATUS]` block now reads "Gemini: SKIPPED — quota-exhausted (resets at UTC midnight) — will attempt substitute via llama-3.1-8b-instant on Groq (Wave 22a/f)" when env-enabled, closing the user-facing loop. Exported `composeWithContext` from engine.ts so `runFanOut` can recompose the system prompt fanOut used internally. 7 new tests in `gemini-substitute-mcp.test.ts` covering preflight messaging + classification. 530/530 tests; qa:check + security:check both clean. Commit `5cee893`.

Earlier today: Wave 22e shipped — doc refresh driven by a real apex_doc_review dogfood pass (manually pipelined via apex_synthesize since this Claude Code session's MCP handshake predates Wave 22c so the new tool isn't visible to the client; server has all 16). Live panel verdict 2026-05-27 confidence 85, Doc Health: **Untrustworthy** before the fix. Resolved drift in CLAUDE.md + README.md: (1) MCP tool count corrected from "seven tools"/"14 tools" → **16 tools** in both files, with the canonical reference now pointing at `src/mcp/register-tools.ts:REGISTERED_TOOL_NAMES` rather than an inline list that re-rots on every wave; (2) DeepSeek added to the fan-out enumeration (4 → 5 providers) in CLAUDE.md "Goal" block, architecture diagram, and README's Intelligence section; (3) QUALITY_SCORE list extended to deepseek=3 to match `src/lib/providers.ts`; (4) Vitest stats refreshed (28 files / 234 tests → 45 / 500+) with a forward-compat note pointing at the tests directory for live counts; (5) `apex_self_check` expected output corrected from "7 tools" → 16; (6) "CC" acronym expanded to "Claude Code (CC)" on first use; (7) "gate run" jargon defined inline; (8) README's Tools section restructured into Headline / Grounded maker-checker reviewers / Infra-utility buckets covering all 16; (9) `CLAUDE.MD` case-sensitivity typo fixed → `CLAUDE.md`. **Real-world Wave 22d validation**: the `[PRE-FLIGHT STATUS]` block fired live during the dogfood call showing "Running 4/5 providers — Gemini: SKIPPED — quota-exhausted (resets at UTC midnight)" exactly as designed — the LFM ask shipped Wave 22d looks correct in practice. No code changes, 523/523 tests still passing, gates clean. Commit `21f2c42`.

Earlier today: Wave 22d shipped — closes the remaining LFM asks from #33 first-half (pre-flight loud-degrade on MCP-side fan-out) and #22 minor ask (per-slot timeout clarity). New `src/lib/preflight-status.ts` (~140 LOC): `buildPreflightStatus({includeClaude})` queries `getAllQuotaStates()` + env-gating + auto-include-Claude-when-2+-exhausted logic to know which providers WILL run BEFORE fan-out fires. `formatPreflightBlock(status)` emits a `[PRE-FLIGHT STATUS]` block with "Running N/M providers", per-provider will-run/SKIPPED lines, and an IMPORTANT degradation warning when N<M. Wired into both `apex_fanout` and `apex_synthesize` — block is prepended to the response BEFORE the synthesizer output / individual answers. Extended `src/lib/panel-status.ts`: added `errorKind` field to `PanelStatusEntry` (classified via `classifyError`) + new `panelStatusEntryLabel(entry)` helper that maps timeout→"timed out", rate-limited→"rate-limited", gemini-quota-exhausted→"quota-exhausted", etc. `formatPanelStatusBlock` now surfaces `UNAVAILABLE (timed out) — <raw>` instead of bare `UNAVAILABLE — operation aborted` (LFM-filed #22 ask exactly). Backward-compat: when `errorKind` is absent (hand-built test entries) falls back to the historical format. 23 new tests across `preflight-status.test.ts` + `panel-status-timeout.test.ts`. 523/523 tests; qa:check + security:check both clean. Closes LFM #33 + #22 + #34 (with thanks). Commit `b6769ad`.

Earlier today: Wave 22c shipped — LFM #32 `apex_doc_review` MCP tool (16th). Prose maker-checker panel — the documentation analogue of `apex_code_review`. Same 5-persona dissent-preserving panel shape, but with **prose-native slots** (different from code-review's logic/approach/security/business-logic/qa): claude→**consistency** (contradictions), openai→**freshness** (stale paths/versions), llama→**cross-refs** (broken navigation), gemini→**clarity** (ambiguity), deepseek→**rationale** (missing "why"). MoA panel verdict 2026-05-27 confidence 85 picked this 5-mode rubric from 10 candidates A-J (rejected: duplication merged into contradiction, audience-mismatch too subjective, tone bikeshedding, structural produces vague findings, outdated-examples merged into staleness). **Resolution Report**: when projectRoot is set, apex pre-resolves every `path/to/file[:symbol]` reference in the doc body (fs.existsSync + symbol grep) and prepends the result to the panel prompt so freshness reviewer cites hard evidence instead of guessing — extracted refs cap at 50 to prevent runaway. Multi-file mode: caller supplies `files: [{path, body}]` directly OR `filePaths` + projectRoot (apex reads via loadReviewFile, same path-traversal guard); up to 5 files concatenated with `--- FILE:` separators. 16k/file truncation cap, hard 32k ceiling per file. **Doc-native severity**: Misleading / Confusing / Polish (NOT code-review's Critical/High/Medium/Low). **Doc Health roll-up**: Trustworthy / Patchy / Untrustworthy. Synth system prompt mirrors code-review's dissent-preserving + evidence-drop rules but adapted for prose. Three new lib files (`src/lib/doc-review-personas.ts`, `doc-review-resolver.ts`, `doc-review-prompt.ts`) ~600 LOC total. 37 new tests across the three modules. 500/500 tests; qa:check + security:check both clean. Commit `0395db0`.

Earlier today: Wave 22b shipped — LFM #31 `apex_read_source` MCP tool (15th). Symmetric to `apex_query_source` (#13, declared data sources) but for SOURCE FILES — TypeScript / Python / Markdown / config. Single tool with `mode` enum (`read` | `list` | `tree`) — MoA verdict 2026-05-27 confidence 80, 2/3 converge on single-tool shape (Claude/Llama; GPT minority for 3 tools). Read mode delegates to existing `loadReviewFile()` (20k cap + line numbers). List mode returns markdown listing, dirs first, 200-entry cap. Tree mode walks recursively with default depth 2, hard cap 4. **Hardcoded denylist** (YAGNI per Claude's recommendation; defer `.apex/read-source.json` to follow-up): rejects path SEGMENTS matching `node_modules`, `.git`, `.next`, `.turbo`, `.vercel`, `build`, `dist`, `out`, `coverage`, `data`, plus any segment starting with `.env`. Segment-match (not substring) — `database.ts` and `datasheet.ts` correctly bypass `data` denylist. Same realpathSync + isInside confinement as `review-file-loader.ts` (symlinks resolve to target; target must stay in projectRoot). 30k total response cap across all modes. New `src/lib/apex-read-source.ts` (~250 LOC) + 21 new tests covering path traversal, symlinks-out, denylist segments (real + substring false-positive), all three modes + truncation footers. 463/463 tests; qa:check + security:check both clean. Commit `4ab037a`.

Earlier today: Wave 22a shipped — LFM #33b Gemini quota-exhaust cross-provider substitute. Symmetric mirror of Wave 20c (openai content-filter → `openai/gpt-oss-120b`): when Gemini's free-tier 429 fires (`free_tier_requests` quotaMetric OR `RESOURCE_EXHAUSTED` from Google AI Studio), route the slot through `llama-3.1-8b-instant` on Groq (Production tier, verified 2026-05-27). New `gemini-quota-exhausted` ErrorKind (narrow: requires BOTH provider==="gemini" marker AND free-tier/RESOURCE_EXHAUSTED marker — generic 429s still classify plain rate-limited and retry). New `streamGeminiQuotaFallback()` in engine.ts mirrors `streamOpenaiContentFilterFallback()` shape (timeout-combined signal, Groq via `streamGroqText`). New substitute branch in route.ts mirrors Wave 20c block (ack-strip reuse, manual iterator for usage capture, env-gated via `APEX_GEMINI_QUOTA_FALLBACK`, model swap in cache key + cost estimation). MoA panel verdict: confidence 78, 3/4 converge on llama-3.1-8b-instant (Claude/Llama primary, GPT alternate-with-fallback); rejected `openai/gpt-oss-20b` to avoid same-release-pair correlation with openai-slot substitute. 6 new tests under "Wave 22a — gemini-quota-exhausted is a distinct kind…". 442/442 tests; qa:check + security:check both clean. Commit `03a9776`.

Earlier today: Wave 21d shipped — 5 Highs from QA agent's review + LFM #33a (pre-flight loud-degrade). (H3) bounded `res.text()` body read in web-fetch — manual stream reader with 4MB cap aborts slowloris-after-headers DoS; (H4) manual redirect chain walking — `redirect: "manual"`, per-hop SSRF re-check, MAX_REDIRECTS=10; (H5) timeout wrapper for `streamOpenaiContentFilterFallback` — `AbortSignal.timeout(DEFAULT_PROVIDER_TIMEOUT_MS)` composed with caller signal so a hung Groq can't wait forever; (H6) manual iteration of the substitute generator to capture its `usage` return value — Groq fallback tokens now billed correctly to acc.inputTokens/outputTokens/costUsd; (H7) sanitize `acc.error` + `acc.substituted.reason` before interpolating into `[PROVIDER STATUS]` — strips block markers, control chars, directive-shaped lines, caps at 300 chars; (LFM #33a) extend `[PROVIDER STATUS]` to also list `NOT RUN` slots with reason (user-disabled / env-gated / did-not-participate) so the synth knows the panel's effective size. New `src/lib/provider-status-sanitize.ts` (9 tests) + 6 new web-fetch H3/H4 integration tests using vi.fn-mocked fetch. 436/436 tests; qa:check + security:check both clean. Commit `e6280b0`.

Earlier today: Wave 21c — 3 Criticals + B1 + LFM #30, commit `423cab1`. — **security hotfix** addressing 3 Criticals + 1 borderline + LFM bug #30. Both MoA subagents (QA + Security) reviewed Waves 19/20/21; this commit fixes their consolidated Critical-tier findings: (C1) IPv6-mapped IPv4 SSRF bypass in `web-fetch.ts` (`[::ffff:127.0.0.1]` form passed isSafePublicHost — live-verified by QA agent); (C2) SQL comma-join allowlist bypass in `apex-source-query.ts` (`FROM allowed, secrets` extracted only `allowed`); (C3) openai content-filter substitute stream bypassed the ack-token strip + grounding-state emit → `[grounded]` leaked to user-visible text + UI badge desync. Plus (B1) `LOAD_EXTENSION`/`RANDOMBLOB`/`WRITEFILE` added to forbidden-keyword regex (defense-in-depth); (H1) DNS cloud-metadata hostnames (`metadata.google.internal` etc.) added to SSRF reject list; (H2) SQL comment-stripping before table extraction; and LFM #30 synth-empty fallback (synth → claude → openai → llama → gemini → deepseek precedence chain when synth produces zero tokens). Extracted ack-token logic to `src/lib/ack-token-strip.ts` so primary + substitute streams share machinery. 420/420 tests (29 new); qa:check + security:check both clean. Commit `423cab1`.

Earlier today: Wave 21b — auto-fetch top web results on grounding, commit `c30325e` + `fba87bc`. — **auto-fetch top web results when grounding fires**. New `webFetchDepth` setting (0-3, default 0 = off). When grounding fires AND depth > 0, apex fetches the top N URLs in parallel via apex_web_fetch and appends their cleaned bodies to the [WEB_CONTEXT] block under a `## Full content of top results` subsection (3000 chars/page cap; failures skipped silently). New `web-fetched` SSE event + UI badge `📄 +N pages` next to the grounded badge on the synth panel (with tooltip listing every fetched URL + char count). Settings UI control: 4-state Off/1/2/3. 391/391 tests; type-check + build clean. Commit `c30325e`.
...
Closed #28 + #29 as fixed in Wave 20c (pre-fix auto-bugs for the openai content-filter pattern Wave 20c addresses).

Web UI relocated to **http://localhost:3010** (port 3000 freed for another app). MCP HTTP unaffected.
