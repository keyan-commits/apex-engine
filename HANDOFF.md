...
> Resume card — read first to pick up in a new session. Volatile state only.
> `CLAUDE.md` is stable architecture; **git log is the full changelog**. Per-wave
> detail lives in commit messages, not in prose here.

**Updated:** 2026-05-28

## Now
Wave 23 shipped (`d868a36`) — provider catalog drift detector (`pnpm catalog:check`,
detect-and-notify). 557/557 tests; qa:check + security:check clean. Web UI on
http://localhost:3010 (3000 freed for another app); MCP HTTP unaffected.

## Next
1. Backlog **12c** — disagreement-driven re-fan-out (~120 LOC; needs 2nd-panel UX).
2. Backlog **12d** — chain-of-verification lite (~150 LOC; claim extract + footnotes).
3. Backlog **B5** — escalation ladder (cheap-first, re-run on low-confidence; B3 ✓).

## Blockers
(none) — all gates clean as of Wave 23. Open: LFM-side validation of Waves 19+20 (no new signal yet).

## Resume
`pnpm dev` (web :3010) · `pnpm mcp:http` (MCP server) · `pnpm qa:check` ·
`pnpm security:check` · `pnpm test:run`
...
```
## Convention
After each wave: replace **Now**, refresh **Next**/**Blockers**, append one row to the
wave table above, write the commit SHA. **SHA backfill:** the SHA doesn't exist until
you commit — write `(SHA-pending)` in the wave row, commit, then `git commit --amend`
(if unpushed) or ONE fixup commit to fill it in. Never a trail of separate "patch Wave
NN SHA" commits. Full per-wave narrative → the commit message + git log, not here.
