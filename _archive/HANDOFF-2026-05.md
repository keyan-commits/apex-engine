...
> **Archived 2026-05-27 — historical, not authoritative.** Moved out of the live `HANDOFF.md` via `/handoff archive` (Wave 27). Captures Wave 7–21 session blocks plus the "Filing-conventions awareness pass" / "Triage ops trio" / "Backlog" / "Pitfalls" / "Resume-from-clean-state commands" / "What's in apex-engine today" / "Engineering decisions worth preserving" / "File layout" / "Commands" reference sections that had drifted into HANDOFF (those belong in `CLAUDE.md` / specs, not the resume card). For current state see the live [`HANDOFF.md`](../HANDOFF.md). For per-wave diffs see `git log`.
